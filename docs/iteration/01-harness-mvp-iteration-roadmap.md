...
| 9a | Polyglot enforcement: edges (light) DONE 2026-06-26 | The harness's boundary checking reads import edges from the CodeGraph index (schema-guarded DB read) instead of Python `ast`, so it works across languages. Sample linter stays `ast`. Proved live on `examples/ts-mini`. |
| 9b | Polyglot enforcement: mechanical gate 2 (light) DONE 2026-06-26 | Read method/function signatures from CodeGraph; make gate 2 a deterministic signature diff and retire the LLM-judged gate 2. Contract field diffing stays `ast` (the index has no field nodes). See [`12-iteration-9b-codegraph-signatures-plan.md`](12-iteration-9b-codegraph-signatures-plan.md). Single-observed-source goal now met for edges (9a) and signatures (9b); contract fields are the lone `ast` holdout. |
